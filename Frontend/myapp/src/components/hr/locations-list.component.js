...
const Location = props => (
  <tr>
    ...
    <td>
      <Link to={"/hr/locations/"+props.location.room}>edit</Link> | <a href="#" onClick={() => { props.deleteLocation(props.location.room) }}>delete</a>
    </td>
  </tr>
)
...
export default class LocationsList extends Component {
  ...
  componentDidMount() { 
    axios.get('http://localhost:1000/hr/location',{headers: {'token':localStorage.getItem("token")}})
      .then(response => {
  ...
  deleteLocation(room) {
    axios.delete('http://localhost:1000/hr/location/'+room,{headers: {'token':localStorage.getItem("token")}})
