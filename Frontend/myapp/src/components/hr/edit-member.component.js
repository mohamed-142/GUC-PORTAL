...
import axios from 'axios'
  ...
  export default class EditMember extends Component {
    ...
    async componentDidMount() { 
        await axios.get('http://localhost:1000/hr/member',{headers: {'token':localStorage.getItem("token")}})
        .then(response => {
    ...
    async onSubmitU(e){
        ...
        await axios.put('http://localhost:1000/hr/member',body).then(res =>{
    ...
    async onSubmitD(e){
       ...
       await axios.delete('http://localhost:1000/hr/member/'+this.state.id).then(res =>{
            console.log(res.data)
        })
        .catch(err=>console.log(err))
